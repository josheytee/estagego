				...
				if ($item->GroupTitle && (!$this->IsRoot || !EW_MENU_ROOT_GROUP_TITLE_AS_SUBMENU)) { // Group title
					...
					if (!is_null($item->SubMenu)) {
						foreach ($item->SubMenu->ItemData as $subitem) {
							...
							if (ew_CurrentPage() == ew_GetPageName($subitem->Url))
					...
					if (ew_CurrentPage() == ew_GetPageName($item->Url)) {
						if ($this->IsRoot)
							ew_AppendClass($liclass, EW_MENU_ACTIVE_ITEM_CLASS);
						else
...
class cMenuItem {
	...
	function Render($aclass = "", $liclass = "", $mobile = FALSE) {
		...
		if ($mobile) {
			...
			if ($url == "") $url = "#";
			$attrs = array("class" => $aclass, "rel" => "external", "href" => $url, "target" => $this->Target);
		} else {
			if ($url == "") $url = "#";
...
function ew_UserAgent() {
	...
	if (preg_match("/AppleWebKit\/([^\s]*)/i", $ua, $m)) { 
		...
		if (preg_match("/ Mobile\//i", $ua)) {
			$mobile = "Apple"; // Check if iPhone or iPod Touch
			if (preg_match("/OS ([^\s]*)/i", $ua, $m)) {
				$os = "iOS";
				$version = str_replace("_", ".", $m[1]); // Get iOS version
			}
