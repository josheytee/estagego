...
class cservices extends cTable {
	...
	function __construct() {
		...
		// id
		$this->id = new cField('services', 'services', 'x_id', 'id', '`id`', '`id`', 21, -1, FALSE, '`id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['id'] = &$this->id;

		// service_name
		$this->service_name = new cField('services', 'services', 'x_service_name', 'service_name', '`service_name`', '`service_name`', 200, -1, FALSE, '`service_name`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['service_name'] = &$this->service_name;

		// description
		$this->description = new cField('services', 'services', 'x_description', 'description', '`description`', '`description`', 200, -1, FALSE, '`description`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['description'] = &$this->description;

		// image
		$this->image = new cField('services', 'services', 'x_image', 'image', '`image`', '`image`', 200, -1, TRUE, '`image`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['image'] = &$this->image;

		// url
		$this->url = new cField('services', 'services', 'x_url', 'url', '`url`', '`url`', 200, -1, FALSE, '`url`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['url'] = &$this->url;

		// created_at
		$this->created_at = new cField('services', 'services', 'x_created_at', 'created_at', '`created_at`', 'DATE_FORMAT(`created_at`, \'%Y/%m/%d\')', 135, 5, FALSE, '`created_at`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->created_at->FldDefaultErrMsg = str_replace("%s", "/", $Language->Phrase("IncorrectDateYMD"));
		$this->fields['created_at'] = &$this->created_at;

		// updated_at
		$this->updated_at = new cField('services', 'services', 'x_updated_at', 'updated_at', '`updated_at`', 'DATE_FORMAT(`updated_at`, \'%Y/%m/%d\')', 135, 5, FALSE, '`updated_at`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
	...
	function ExportDocument(&$Doc, &$Recordset, $StartRec, $StopRec, $ExportPageType = "") {
		...
		if ($Doc->Horizontal) { // Horizontal format, write header
			...
			if ($ExportPageType == "view") {
				...
				if ($this->service_name->Exportable) $Doc->ExportCaption($this->service_name);
		...
		while (!$Recordset->EOF && $RecCnt < $StopRec) {
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if ($ExportPageType == "view") {
					...
					if ($this->service_name->Exportable) $Doc->ExportField($this->service_name);
