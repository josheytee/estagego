...
class ccategories extends cTable {
	...
	function __construct() {
		...
		// id
		$this->id = new cField('categories', 'categories', 'x_id', 'id', '`id`', '`id`', 19, -1, FALSE, '`id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['id'] = &$this->id;

		// category_name
		$this->category_name = new cField('categories', 'categories', 'x_category_name', 'category_name', '`category_name`', '`category_name`', 200, -1, FALSE, '`category_name`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['category_name'] = &$this->category_name;

		// page_id
		$this->page_id = new cField('categories', 'categories', 'x_page_id', 'page_id', '`page_id`', '`page_id`', 19, -1, FALSE, '`page_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->page_id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['page_id'] = &$this->page_id;

		// url
		$this->url = new cField('categories', 'categories', 'x_url', 'url', '`url`', '`url`', 200, -1, FALSE, '`url`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['url'] = &$this->url;

		// created_at
		$this->created_at = new cField('categories', 'categories', 'x_created_at', 'created_at', '`created_at`', 'DATE_FORMAT(`created_at`, \'%Y/%m/%d\')', 135, 5, FALSE, '`created_at`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->created_at->FldDefaultErrMsg = str_replace("%s", "/", $Language->Phrase("IncorrectDateYMD"));
		$this->fields['created_at'] = &$this->created_at;

		// updated_at
		$this->updated_at = new cField('categories', 'categories', 'x_updated_at', 'updated_at', '`updated_at`', 'DATE_FORMAT(`updated_at`, \'%Y/%m/%d\')', 135, 5, FALSE, '`updated_at`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			return $this->KeyUrl("categoriesview.php", $this->UrlParm($parm));
		else
			return $this->KeyUrl("categoriesview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	}

	// Add URL
	function GetAddUrl() {
		return "categoriesadd.php";
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		return $this->KeyUrl("categoriesedit.php", $this->UrlParm($parm));
	}
	...
	function GetCopyUrl($parm = "") {
		return $this->KeyUrl("categoriesadd.php", $this->UrlParm($parm));
	}
	...
	function ExportDocument(&$Doc, &$Recordset, $StartRec, $StopRec, $ExportPageType = "") {
		...
		if ($Doc->Horizontal) { // Horizontal format, write header
			...
			if ($ExportPageType == "view") {
				...
				if ($this->category_name->Exportable) $Doc->ExportCaption($this->category_name);
		...
		while (!$Recordset->EOF && $RecCnt < $StopRec) {
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if ($ExportPageType == "view") {
					...
					if ($this->category_name->Exportable) $Doc->ExportField($this->category_name);
